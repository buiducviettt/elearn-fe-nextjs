...
import { HIGHLIGHT_CLASS } from "./components/HighlightManager";
...
// Hàm loại bỏ /wp-elearn khỏi các src ảnh
function cleanImageUrls(content: string) {
    return content.replace(/src="([^"]*?)\/wp-elearn([^"]*?)"/g, 'src="$1$2"');
}
...
const MyEditor: React.ForwardRefRenderFunction<TMyEditorRef, TMyEditorProps> = (
    ...
    // Xử lý value truyền vào để loại bỏ /wp-elearn khỏi src ảnh
    const finalValue = useMemo(() => {
        return value ? cleanImageUrls(value) : insideValue;
    }, [value, insideValue]);
    ...
    const insertFillingInput = () => {
        ...
        const html = `
            <span class="input-inline" data-id="ip_${nextIndex}">
        ...
        >
            ...
            <TinyMCEEditor
                ...
                init={{
                    ...
                    toolbar:
                        "undo redo | blocks | bold italic underline strikethrough lineheight | table | " +
                    ...
                    images_upload_handler: async (blobInfo: any) => {
                        ...
                        try {
                            ...
                            let url = res?.payload?.source_url;
                            if (!url) throw new Error("Upload failed");
                            // Xóa /wp-elearn nếu có trong url
                            url = url.replace("/wp-elearn", "");
                            return url;
                        } finally {
